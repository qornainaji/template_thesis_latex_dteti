...
kesenjangan yang serius dalam keterampilan \textit{Self-Regulated Learning} (SRL), yang dapat 
...
Untuk mengatasi kesenjangan SRL, Zimmerman mengemukakan kerangka kerja 
yang mencakup tahap 
perencanaan (\textit{forethought}), pelaksanaan (\textit{performance}), 
dan refleksi diri 
(\textit{self-reflection}) 
di mana SRL merupakan gabungan strategi kognitif, motivasional, 
dan perilaku \cite{liu2025srl, akdeniz2022srl}. 
Individu dengan kemampuan regulasi diri yang efektif terbentuk 
melalui penetapan tujuan, perancangan strategi, 
pemantauan diri secara aktif, dan evaluasi diri yang bijaksana. 
Sejumlah besar penelitian 
mengonfirmasi hubungan kuat antara keterampilan ini dan 
pencapaian akademik \cite{liu2025srl, xiao2019relationships}. 
Namun, pengembangannya kerap terhambat oleh faktor personal, 
kontekstual, dan sosial, mulai dari manajemen waktu yang buruk 
dan distraksi digital hingga rendahnya kesadaran metakognitif.
Metakognisi sendiri merupakan pengetahuan, kesadaran, dan 
regulasi atas pemikiran diri sendiri, yang berperan penting 
dalam membangun kesadaran mahasiswa akan proses belajarnya
\cite{zimmerman2009selfregulation}.
Hambatan tersebut tersebut menyebabkan intervensi terstruktur 
diperlukan untuk membimbing 
mahasiswa menjalani proses yang kompleks ini 
\cite{chitra2022srl, 10.3389/feduc.2024.1418297}.
% ------------------------------------------------

Metode tradisional yang paling efektif untuk mengembangkan keterampilan SRL adalah umpan balik 
...
merupakan katalis kuat untuk perbaikan [15, 16]\cite{10.3389/fpsyg.2022.1027266, Bock14032024}. 
Namun "standar emas" ini menghadapi krisis 
skalabilitas dengan meningkatnya jumlah mahasiswa dan kebutuhan dukungan yang beragam membuat 
model bimbingan secara personal sulit diterapkan dalam skala besar. Hal ini diperkuat
...
Salah satu teknologi tersebut adalah penggunaan \textit{Large Language Models} (LLM). LLM dapat 
...
Rekomendasi tersebut menegaskan bahwa respons LLM masih membutuhkan pengawasan pakar agar relevan.
...
Oleh karena itu, skripsi ini berfokus pada peningkatan kualitas performa respons 
LLM sebagai pemberi umpan balik agar mendekati mutu ahli manusia secara kuantitatif 
dan kualitatif. Peningkatan dilakukan melalui kerangka evaluasi yang menggabungkan 
acuan ahli manusia sebagai \textit{ground truth} dan penilaian kualitas yang menilai 
dimensi-dimensi pedagogis relevan \cite{Usher2025GenAIAssessments, Maurya2025UnifyingAITutorEvaluation}. 
% -------------------------------------------------
...
papan Kanban dipakai sebatas alat operasional untuk menata dan mengekstraksi
data proses (perpindahan kartu, checklist, waktu pengerjaan) yang dapat dipetakan
ke fase SRL (perencanaan-\textit{monitoring}-refleksi) \cite{strickroth2022kanban}. 
Penggunaan papan Kanban
hanya sebagai salah satu instrumen pengaya data agar evaluasi dan perbaikan kualitas 
umpan balik LLM bisa berjalan lebih terarah. Pada penelitian ini,
dijelaskan pada bab berikutnya bahwa fokus kebaruan terletak pada rangkaian 
evaluasi dan perbaikan \textit{prompt} LLM. Penelitian ini akan memperbaiki
tiga fungsi pedagogis utama yakni feedback, 
dukungan motivasional, dan dukungan apresiatif dengan
...
Secara spesifik, penelitian ini berfokus pada 
pengembangan umpan balik yang 
menargetkan tiga fungsi pedagogis yaitu \textit{feedback}, 
\textit{motivation}, dan 
\textit{appreciation}. Rumusan masalah yang diangkat 
dalam penelitian ini adalah 
sebagai berikut:
\begin{enumerate}
  \item Belum layaknya kualitas umpan balik LLM terhadap proses 
  SRL mahasiswa yang direpresentasikan melalui data papan Kanban
  jika dibandingkan dengan umpan balik ahli manusia.

  % \item Belum diketahui sejauh mana kualitas umpan balik 
  % yang dihasilkan LLM terhadap proses SRL mahasiswa yang 
  % direpresentasikan melalui data papan Kanban dapat mendekati 
  % atau setara dengan kualitas umpan balik ahli manusia, 
  % baik secara kuantitatif maupun kualitatif.
  \item Butuhnya metode evaluasi yang melibatkan acuan ahli manusia
  sebagai \textit{ground truth} dan penilaian kualitas yang menilai 
  dimensi-dimensi pedagogis relevan untuk mengukur
  kualitas umpan balik LLM terhadap proses SRL mahasiswa.

  \item Belum jelas pengaruh ukuran model LLM terhadap 
  kualitas umpan balik yang dihasilkan di mana semakin
  besar ukuran model LLM, semakin mahal biayanya. 
\end{enumerate}


% \begin{enumerate}
%   \item \textit{Umpan Balik Kognitif}: Bagaimana membimbing 
% mahasiswa melalui proses belajar 
% (mendukung \textit{ZPD} mereka) tanpa memberikan 
% jawaban langsung.

%   \item \textit{Dukungan Motivasional}: Bagaimana memberikan 
% dorongan yang tulus untuk meningkatkan 
% \textit{self-efficacy} mahasiswa saat mereka 
% menghadapi kesulitan.

%   \item \textit{Dukungan Apresiatif}: Bagaimana 
%   memvalidasi usaha dan pencapaian (bukan hanya hasil) 
%   untuk memperkuat perilaku belajar yang positif.
% \end{enumerate}

% Penelitian ini berfokus pada masalah pengembangan 
% dan evaluasi strategi intervensi 
% (berbasis \textit{prompt}) untuk meningkatkan 
% performa LLM dalam ketiga fungsi tersebut, dengan 
% menggunakan jejak data pembelajaran mahasiswa 
% (dari papan Kanban) sebagai pemicu umpan balik.
...
\label{subsection:tujuan_penelitian}
...
Sistematika penulisan disusun untuk memberikan gambaran menyeluruh 
mengenai alur pembahasan dalam skripsi ini. Penulisan skripsi ini 
dibagi menjadi lima bab yang saling berkaitan, dengan rincian 
sebagai berikut:

\vspace{1em} % Memberi jarak antar paragraf
\clearpage
\noindent \textbf{BAB I: PENDAHULUAN} \\
Bab ini membahas dasar pemikiran dan kerangka awal penelitian. Di 
dalamnya, diuraikan mengenai latar belakang masalah yang mendasari 
penelitian, rumusan masalah yang akan diselesaikan, tujuan penelitian, 
batasan masalah agar pembahasan lebih terarah, manfaat penelitian bagi 
berbagai pihak, serta sistematika penulisan laporan ini.

\vspace{1em}

\noindent \textbf{BAB II: TINJAUAN PUSTAKA DAN DASAR TEORI} \\
Bab ini menyajikan kajian literatur dan landasan teori yang relevan 
dengan topik penelitian. Bagian tinjauan pustaka memaparkan penelitian 
terdahulu mengenai peningkatan kualitas umpan balik LLM, 
\textit{Self-Regulated Learning} (SRL), dan penggunaan 
\textit{Learning Analytics}. Bagian dasar teori menjelaskan 
konsep-konsep kunci seperti Psikologi Pendidikan (SRL, Teori 
Umpan Balik, \textit{Self-Determination Theory}), \textit{Large 
Language Models} (LLM), \textit{Prompt Engineering}, \textit{Context 
Engineering}, Papan Kanban, serta metrik evaluasi seperti 
\textit{Theory-driven prompt manual}, BERTScore, 
BARTScore, dan LLM-as-a-Judge. Pertanyaan penelitian (\textit{Research 
Questions}) juga dirumuskan dalam bab ini.

\vspace{1em}

\noindent \textbf{BAB III: METODE PENELITIAN} \\
Bab ini menguraikan metodologi yang digunakan untuk mencapai 
tujuan penelitian. Pembahasan mencakup alat dan bahan penelitian, 
metode pembuatan data sintetis papan Kanban, metode peningkatan 
kualitas umpan balik melalui rekayasa konteks dan integrasi 
\textit{Learning Analytics}, serta metode evaluasi baik secara 
kuantitatif maupun kualitatif. Alur penelitian dijelaskan 
secara sistematis mulai dari persiapan hingga analisis akhir.

\vspace{1em}

\noindent \textbf{BAB IV: HASIL DAN PEMBAHASAN} \\
Bab ini memaparkan hasil eksperimen dan analisis mendalam terhadap 
data yang diperoleh. Pembahasan meliputi ringkasan pelaksanaan 
eksperimen, analisis perbandingan metode peningkatan umpan balik 
untuk menjawab pertanyaan penelitian pertama, analisis pengaruh 
ukuran model LLM untuk menjawab pertanyaan penelitian ketiga, serta 
validasi dan persepsi ahli manusia untuk menjawab pertanyaan 
penelitian kedua. Bab ini juga membahas kelebihan dan kekurangan 
dari penelitian yang dilakukan.

\vspace{1em}

\noindent \textbf{BAB V: KESIMPULAN DAN SARAN} \\
Bab penutup ini berisi kesimpulan yang ditarik berdasarkan hasil 
analisis dan pembahasan untuk menjawab rumusan masalah. Selain itu, 
bab ini juga menyajikan saran-saran konstruktif untuk pengembangan 
