...
\texttt{stuck}, dan \texttt{alerts} yang diekstraksi dari 
...
\begin{figure}[H]
  ...
  \caption{Diagram Alir Metode Evaluasi.}
...
\begin{figure}[H]
  ...
  \caption{Flowchart Alur Penelitian Tugas Akhir.}
...
topik penelitian. Tinjauan ini berfokus pada penelitian-penelitian 
...
\begin{figure}[H]
  ...
  \caption{Tampilan Utama Platform Gamatutor.}
...
\begin{figure}[H]
  ...
  \caption{Tampilan Informasi Tiap Kartu Gamatutor.}
...
\begin{figure}[H]
  ...
  \caption{Skema Penyimpanan Data Kartu Pembelajaran Kanban Mahasiswa.}
...
\begin{figure}[H]
  ...
  \caption{Skema Penyimpanan Data Kartu Pembelajaran Kanban Mahasiswa Setelah Reduksi}
...
snipet pada Gambar \ref{Fig:contoh_prompt}.
...
\begin{figure}[H]
  \centering
	\caption{Contoh \textit{Prompt} untuk Studi Kasus Data Sintetis Mahasiswa.}
  \label{Fig:contoh_prompt}
    \begin{lstlisting}
"""
...
dapat dilihat pada Gambar \ref{Fig:prompt_batasan_subtopik}.
...
%   \begin{minipage}{\textwidth}
    ...
    % Mulai listing Anda (tanpa caption/label di sini)
% \begin{lstlisting}[label={tab:prompt_batasan_sub}, caption={\textit{Prompt} untuk Batasan Subtopik Materi Pembelajaran Mahasiswa}]
\begin{figure}[H]
  \centering
	\caption{\textit{Prompt} untuk Batasan Subtopik Materi Pembelajaran Mahasiswa.}
  \label{Fig:prompt_batasan_subtopik}
\begin{lstlisting}
...
\textit{prompt} dipaparkan pada Gambar \ref{Fig:prompt_role_play}.
...
\textit{Prompt} yang ditunjukkan pada Gambar \ref{Fig:prompt_role_play}
...
pada Gambar \ref{Fig:prompt_few_shot} bentuk struktur JSON yang 
diberikan ke model.
...
\begin{figure}[H]
  \centering
	\caption{\textit{Few-Shot Prompt} pada Pembuatan Data Sintetis.}
  \label{Fig:prompt_few_shot} % Saya ubah labelnya menjadi
		% Mulai listing Anda (tanpa caption/label di sini)
		\begin{lstlisting}
...
struktur Kanban yang diberikan pada Gambar \ref{Fig:prompt_struktur_kanban}
...
\begin{figure}[H]
  \centering
	\caption{\textit{Prompt} Struktur Papan Kanban.}
  \label{Fig:prompt_struktur_kanban} % Saya ubah labelnya
		% Mulai listing Anda (tanpa caption/label di sini)
		\begin{lstlisting}
...
\begin{figure}[H]
  \centering
	\caption{CoT \textit{Prompt} pada Pembuatan Data Sintetis.}
  \label{Fig:prompt_cot} % Saya ubah labelnya menjadi
    % Mulai listing Anda (tanpa caption/label di sini)
    \begin{lstlisting}
...
Gambar \ref{Fig:prompt_cot} memberikan "Contoh 
...
\begin{figure}[H]
  ...
  \caption{Diagram Alur Urutan \textit{Prompt} Pembuatan Data Sintetis.}
...
yang berjumlah 3 orang yang memiliki latar belakang sebagai
dosen di universitas untuk diberikan penilaian umpan baliknya. 
Ahli manusia tersebut adalah
WWahyu Nur Hidayat (Dosen IT Universitas Malang), Resti Dian Luthvianti
(Dosen SV Universitas Sebelas Maret), dan Asa Hari Wibowo 
(Dosen IT Universitas Halu Oleo).
Umpan balik yang
...
dalam penelitian akademis pendidikan. Berikut adalah tabel 
harga dari model-model yang disediakan oleh Groq API yang 
dapat dilihat pada Tabel \ref{tab:harga_groq_api}.

\begin{table}[H]
	\centering
	\caption{Harga dan Spesifikasi Model (Groq API)}
	\label{tab:harga_groq_api}
	\scriptsize
	\begingroup
	...
	\begin{tabularx}{\textwidth}{@{} X c c c c c @{}}
		\toprule
		\textbf{Model (Konfigurasi)} & \textbf{TPS} & \textbf{Input (\$/token)} & \textbf{Input (tokens/\$1)} & \textbf{Output (\$/token)} & \textbf{Output (tokens/\$1)} \\
		\midrule
	...
	\vspace{4pt}
	% \scriptsize{Catatan: nilai ``tokens/\$1'' sesuai keterangan sumber layanan; tanda bintang pada sumber asli diabaikan di sini.}
\end{table}

Groq API menyediakan 
kedua model dengan biaya
USD 0.05-0.08 per juta token untuk 8B dan USD 0.59-0.79 per 
juta token untuk 70B. Harga model Llama 3.3 70B bukanlah yang paling murah.
Namun, untuk model berukuran besar, Llama 3.3 70B
merupakan model dengan harga yang relatif paling masuk akal untuk 
...
Diketahui juga bahwa sejauh ini belum ada studi komparatif yang
...
Dengan begitu, terdapat kebutuhan untuk
 menyelidiki bagaimana perbedaan kapasitas
...
tujuan pembelajaran mahasiswa. Gambar \ref{Fig:prompt_definisi} menunjukkan
...
\begin{figure}[H]
	  \centering
	  \caption{Contoh Potongan \textit{Prompt} Definisi Peran dan Batasan Agen Penilai.}
	  \label{tab:prompt_definisi}
    % Mulai listing Anda (tanpa caption/label di sini)
    \begin{lstlisting}
		INITIAL_PROMPT = (
...
kolom dalam jangka waktu tertentu. Gambar \ref{Fig:prompt_kriteria} menunjukkan
...
\begin{figure}[H]
	  \centering
	  \caption{Contoh Potongan \textit{Prompt} Kriteria Mahasiswa.}
	  \label{Fig:prompt_kriteria}
...
Gambar \ref{Fig:prompt_fase} menunjukkan
contoh potongan \textit{prompt} yang digunakan untuk menyajikan
...
\begin{figure}[H]
  ...
  \label{Fig:prompt_fase} % Saya ubah labelnya menjadi "tab:"
  % Gunakan minipage untuk "membungkus" listing dengan aman
%   \begin{minipage}{\textwidth}
...
Gambar \ref{Fig:prompt_listmateri} menunjukkan
contoh potongan \textit{prompt} yang digunakan untuk menyajikan
...
\begin{figure}[H]
	  \centering
	  \caption{Contoh Potongan \textit{Prompt} Daftar Materi Mahasiswa.}
	  \label{Fig:prompt_listmateri}
...
Gambar \ref{Fig:prompt_penilaian} menunjukkan
contoh potongan \textit{prompt} yang digunakan untuk menyajikan
...
%   % Gunakan minipage untuk "membungkus" listing dengan aman
%   \begin{minipage}{\textwidth}
\begin{figure}[H] % Anda bisa ganti [H] dengan [htbp] jika perlu
  \centering
  \caption{\textit{Prompt} Penilaian Umpan Balik Pedagogis}
  \label{Fig:prompt_penilaian} % Saya ubah labelnya menjadi "
...
Gambar \ref{Fig:prompt_format_input_output}
...
\begin{figure}[H] % Anda bisa ganti [H] dengan [htbp] jika perlu
  ...
  \label{Fig:prompt_format_input_output} % Saya ubah labelnya menjadi "tab:"
  % Gunakan minipage untuk "membungkus" listing dengan aman
%   \begin{minipage}{\textwidth}
...
ke proses penyusunan ringkasan seperti pada Gambar \ref{Fig:la_indikator_ringkas}
...
\begin{figure}[H] % Anda bisa ganti [H] dengan [htbp] jika perlu
  \centering
  \caption{Contoh Potongan Ringkasan Analitik Mahasiswa dalam Format JSON}
  \label{Fig:la_indikator_ringkas}
...
"Lis Materi" seperti yang tertera pada Gambar \ref{Fig:prompt_listmateri}. 
...
seperti yang tertera pada Gambar \ref{Fig:prompt_listmateri}.
Variasi ketiga adalah \textit{Prompt} yang memasukkan
data analitik dari \textit{learning analytics}
seperti yang tertera pada Gambar \ref{Fig:la_indikator_ringkas}.
...
kualitas umpan balik pedagogis yang dihasilkan oleh
penilai ahli manusia bernama Shafira Anissa yang 
memiliki latar belakang di bidang Psikologi Pendidikan. 
