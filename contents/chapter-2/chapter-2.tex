...
LLM. Metode yang digunakan adalah 
...
\begin{table}[htbp]
\centering
\caption{\textit{Theory-driven prompt manual} untuk memastikan pengembangan \textit{prompt} berkualitas tinggi.}
...
\textbf{evaluasi} yang di mana dijelaskan metode 
penilaian \textit{prompt} dengan \textit{Theory-driven prompt manual}, 
serta metode penilaian umpan balik yang 
menggabungkan metrik kesamaan 
...
\subsubsection{\textit{Reasoning and Acting} (ReAct)}
ReAct (\textit{Reason and Acting}) merupakan sebuah paradigma yang 
menyelaraskan kemampuan penalaran (\textit{reasoning}) dan 
tindakan (\textit{acting}) dalam LLM 
untuk menyelesaikan tugas pemrosesan bahasa maupun 
pengambilan keputusan interaktif. Pendekatan ini 
memperluas ruang aksi agen dengan menggabungkan 
tindakan spesifik tugas dengan tindakan dalam ruang 
bahasa yang disebut sebagai jejak penalaran atau 
\textit{thoughts}. Secara operasional, ReAct meminta LLM untuk 
menghasilkan jejak penalaran verbal dan tindakan secara 
bergantian (\textit{interleaved}), yang memungkinkan model untuk 
melakukan penalaran dinamis guna membuat, memelihara, 
dan menyesuaikan rencana tingkat tinggi sambil berinteraksi 
dengan lingkungan eksternal untuk mendapatkan informasi 
tambahan. Keselarasan ini menciptakan siklus di mana model 
menggunakan penalaran untuk mengarahkan tindakan 
(\textit{reason to act}), dan menggunakan hasil tindakan untuk 
memperkaya penalaran (\textit{act to reason}).

Dalam mekanisme ini, jejak penalaran berfungsi sebagai 
tindakan internal yang tidak memengaruhi lingkungan 
luar, melainkan digunakan untuk menyusun informasi yang 
berguna dengan menalar konteks saat ini, melacak 
kemajuan tugas, serta menangani situasi pengecualian. 
Sementara itu, komponen tindakan memungkinkan model untuk 
menjembatani kesenjangan informasi internal dengan 
berinteraksi langsung dengan sumber eksternal, seperti 
basis pengetahuan atau lingkungan fisik, yang kemudian 
menghasilkan umpan balik berupa observasi nyata. Kombinasi 
ini mengatasi keterbatasan metode 
CoT 
yang statis dan terisolasi, karena ReAct memungkinkan 
model untuk memperbarui pengetahuannya secara reaktif, 
mengurangi halusinasi, serta menghasilkan lintasan 
penyelesaian masalah yang lebih dapat diinterpretasikan 
dan dipercaya oleh manusia.

Dibandingkan dengan metode lain, ReAct mengatasi kelemahan 
mendasar pada CoT yang bersifat 
"kotak hitam" statis dan rentan terhadap halusinasi fakta 
serta propagasi kesalahan karena hanya mengandalkan 
representasi internal, dengan cara memungkinkan 
interaksi langsung dengan lingkungan eksternal 
(seperti API Wikipedia) untuk memperoleh informasi 
faktual yang membumi (\textit{grounded}). Selain itu, ReAct
juga mengungguli pendekatan \textit{Act-Only} yang kerap gagal 
melakukan penalaran abstrak mengenai tujuan tingkat 
tinggi atau kehilangan jejak dalam konteks yang kompleks, 
di mana kegagalan tanpa penalaran tersebut sering kali 
berujung pada kesalahan tindakan yang berulang atau 
halusinasi tindakan.

Berikut adalah representasi gambar diagram alur ReAct dalam 
format \textit{Command Line Interface} (CLI) berdasarkan contoh 
kasus HotpotQA yang terdapat dalam paper \cite{Yao2023}
yang ditunjukkan pada Gambar \ref{fig:react-diagram}.
...
\begin{figure}[H]
    ...
    \begin{lstlisting}[basicstyle=\ttfamily\scriptsize,breaklines=true]
        ...
        > TASK COMPLETED.
        > RESULT: keyboard function keys (Correct)
    \end{lstlisting}
    \caption{Diagram alur ReAct dalam format \textit{Command Line Interface} (CLI) berdasarkan contoh kasus HotpotQA \cite{Yao2023}.}
    \label{fig:react-diagram}
\end{figure}
...
berbeda. Berikut adalah perbedaan utama keduanya [3] yang ditampilkan pada Tabel \ref{tab:perbandingan}.
...
\subsection{Papan Kanban}
Kanban didefinisikan sebagai pendekatan manajemen untuk 
layanan pekerjaan berpengetahuan (\textit{knowledge work}) yang beroperasi 
berdasarkan sistem ``\textit{pull}'' (\textit{pull system}) untuk membatasi 
pekerjaan yang sedang berlangsung (\textit{Work in Progress} (WIP)) 
dan memaksimalkan aliran nilai kepada pelanggan. Metode ini 
dibangun di atas sembilan nilai dasar termasuk transparansi, 
keseimbangan, dan fokus pelanggan. Kemudian metode ini dipandu oleh 
prinsip-prinsip manajemen perubahan yang menekankan evolusi 
bertahap dari proses yang sudah ada tanpa melakukan perombakan 
drastis di awal. Selain itu, prinsip penyampaian layanan 
Kanban menuntut organisasi untuk berfokus pada kebutuhan 
pelanggan, mengelola pekerjaan itu sendiri alih-alih mengelola 
orang-orangnya, dan secara rutin meninjau jaringan layanan 
untuk memastikan efektivitas.


Untuk implementasi praktis, Kanban menerapkan enam praktik 
umum, yaitu memvisualisasikan pekerjaan, membatasi WIP, 
mengelola alur, membuat kebijakan proses menjadi eksplisit, 
menerapkan siklus umpan balik (\textit{feedback loops}), dan melakukan 
peningkatan secara kolaboratif. Penerapan praktik ini didukung 
oleh pemahaman metrik aliran, khususnya \textit{Hukum Little} (\textit{Little's Law}), yang menyatakan bahwa waktu penyelesaian 
rata-rata (\textit{lead time}) berbanding lurus dengan rata-rata WIP 
dibagi dengan rata-rata \textit{throughput}, sehingga pengurangan 
WIP menjadi kunci untuk mempercepat layanan. Dengan 
menggabungkan praktik dan metrik ini, Kanban bertujuan 
mencapai tiga agenda utama: keberlanjutan kerja, orientasi 
layanan yang lebih baik, dan kelangsungan hidup organisasi 
melalui adaptasi yang terus-menerus.

\begin{table}[htbp]
\centering
\caption{Contoh papan Kanban: kelas layanan dan status pekerjaan}
\label{tab:kanban-swimlane}
\begin{tabularx}{\textwidth}{@{}p{3.2cm}*{5}{>{\raggedright\arraybackslash}X}@{}}
\toprule
Swimlane (Kelas Layanan) & Ide / Opsi (Input) & Komitmen (Ready) & Pengembangan (Dev) & Pengujian (Test) & Selesai (Deployed) \\
\midrule
WIP Limit (Batas) & Tak Terbatas & 3 & 2 & 1 & Tak Terbatas \\
\addlinespace
Expedite (Darurat) & --- & --- & [Bug Kritis A] & --- & [Hotfix Server] \\
\addlinespace
Standard (Normal) & [Fitur X] \quad [Fitur Y] & [Fitur Z] \quad [Update UI] & [API Login] (Sedang dikerjakan) \quad [Halaman Admin] (Selesai/Siap tarik) & [Integrasi Payment] & [Rilis v1.0] \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{\textit{Learning Analytics} (LA)}
Learning Analytics (LA) didefinisikan sebagai proses 
pengukuran, pengumpulan, analisis, dan pelaporan data 
mengenai peserta didik serta konteks pembelajaran 
mereka. Tujuan utama dari disiplin ini adalah untuk 
memahami dan mengoptimalkan proses pembelajaran dan 
lingkungan tempat pembelajaran tersebut berlangsung. 
Dalam kerangka teknologi pendidikan yang muncul, LA 
dianggap sebagai instrumen vital yang memanfaatkan 
jejak data digital siswa untuk memberikan wawasan yang 
dapat ditindaklanjuti bagi pendidik guna meningkatkan 
efektivitas pengajaran \cite{Crompton2020}.

Kegunaan utama LA terletak pada kemampuannya untuk 
mendukung personalisasi pembelajaran dan intervensi 
dini. Secara spesifik, LA digunakan untuk melacak 
perilaku dan pola belajar siswa, memberikan umpan balik 
yang membantu mereka mengatur strategi belajar sendiri 
(Self-Regulated Learning). Selain itu, data analitik ini
memungkinkan instruktur untuk mengidentifikasi siswa 
yang berisiko mengalami kesulitan akademik lebih awal, 
sehingga instruksi yang adaptif dan dukungan yang tepat 
dapat diberikan untuk memaksimalkan kesuksesan mahasiswa \cite{Crompton2020}.

Learning Analytics (LA) mengadopsi beragam metode 
yang bersumber dari pendekatan multidisiplin, meliputi 
pembelajaran mesin (machine learning), kecerdasan buatan, 
statistik, temu kembali informasi (information retrieval), 
dan visualisasi. Integrasi metode-metode ini bertujuan 
untuk menganalisis data pendidikan guna mendukung dan 
meningkatkan kualitas proses pembelajaran \cite{chatti_reference_2012}. 
Berdasarkan berbagai literatur penelitian, metode dalam 
learning analytics dapat dikategorikan secara luas ke 
dalam beberapa jenis utama berikut.

\begin{enumerate}
    \item \textbf{Analitik Deskriptif (\textit{Descriptive Analytics})} \\
    Metode ini berfokus pada analisis data pendidikan historis 
    untuk mendeskripsikan perilaku siswa, pola pembelajaran, dan 
    kinerja akademik. Pendekatan ini mencakup penggunaan teknik 
    visualisasi dan analisis statistik untuk memberikan pemahaman 
    mengenai apa yang telah terjadi dalam lingkungan pembelajaran 
    \cite{yunita_research_2021}.
    
    \item \textbf{Analitik Prediktif (\textit{Predictive Analytics})} \\
    Kategori ini memanfaatkan teknik pembelajaran mesin 
    dan penggalian data (\textit{data mining}) untuk memprediksi 
    hasil pembelajaran di masa depan, seperti kinerja 
    siswa, tingkat retensi, probabilitas putus sekolah 
    (\textit{dropout}), atau penyelesaian kursus. Model-model 
    yang umum diterapkan meliputi algoritma klasifikasi, 
    model regresi, dan deep learning yang diaplikasikan 
    pada fitur-fitur yang diekstraksi dari dataset 
    pendidikan \cite{sghir_recent_2022, daud_predicting_2017}.
    
    \item \textbf{Analitik Preskriptif (\textit{Prescriptive Analytics})} \\
    Sebagai pengembangan lebih lanjut dari analitik 
    prediktif, metode ini bertujuan untuk merekomendasikan 
    intervensi atau tindakan nyata guna meningkatkan 
    hasil pembelajaran berdasarkan prediksi yang 
    diperoleh. Fokus utamanya melibatkan perancangan 
    kerangka kerja otomatis untuk menyarankan jalur 
    pembelajaran yang dipersonalisasi (\textit{personalized 
    learning pathways}) atau tindakan pedagogis bagi 
    pengajar \cite{yunita_research_2021}.
    
    \item \textbf{\textit{Educational Data Mining} (EDM)} \\
    Meskipun memiliki keterkaitan erat dan terkadang beririsan 
    dengan LA, EDM memfokuskan diri secara spesifik pada 
    penerapan teknik penggalian data untuk mendeteksi 
    pola-pola tersembunyi dalam data pendidikan. Wawasan 
    yang dihasilkan meliputi penilaian pembelajaran siswa, 
    analisis perilaku, dan akuisisi keterampilan. Teknik 
    EDM yang sering digunakan mencakup klasterisasi 
    (\textit{clustering}), penggalian aturan asosiasi (\textit{association 
    rule mining}), dan penemuan pola 
    \cite{siemens_learning_2012,buenanofernandez_use_2019}.
\end{enumerate}

\subsection{Theory-driven prompt manual}
Dalam konteks pemanfaatan \textit{Generative AI} di 
pendidikan tinggi, kualitas luaran yang dihasilkan 
oleh \textit{Large Language Model} (LLM) sangat bergantung 
pada kualitas instruksi atau \textit{prompt} yang 
diberikan. \textit{Prompt engineering} didefinisikan 
sebagai proses perancangan pertanyaan atau stimulus 
yang efektif untuk menyempurnakan interaksi dengan 
LLM agar menghasilkan respons yang relevan dan akurat
 \cite{jacobsen2025promises}.

Untuk memastikan validitas instrumen prompt yang 
digunakan dalam penelitian ini, disusun sebuah manual 
evaluasi kualitas prompt (\textit{prompt quality manual}) 
yang bersifat \textit{theory-driven}. Pengembangan 
manual ini mengintegrasikan tiga kerangka kerja 
teoritis utama dalam literatur \textit{prompt engineering}, yaitu:
\begin{enumerate}
    \item \textbf{Elemen Modular Kipp:} Mengidentifikasi empat elemen inti prompt yaitu konteks, pertanyaan, format, dan contoh sebagai pedoman konstruksi modular.
    \item \textbf{Faktor Seleksi Ekin:} Menekankan pada lima faktor yang mempengaruhi efektivitas prompt, meliputi niat pengguna, pemahaman model, spesifisitas domain, kejelasan, dan batasan (\textit{constraints}).
    \item \textbf{Kerangka Kerja CLEAR (Lo):} Digunakan sebagai validasi akhir untuk memastikan prompt memenuhi kriteria \textit{Concise} (ringkas), \textit{Logical} (logis), \textit{Explicit} (eksplisit), \textit{Adaptive} (adaptif), dan \textit{Reflective} (reflektif).
\end{enumerate}

Sintesis dari ketiga pendekatan tersebut menghasilkan 
delapan kategori kualitas prompt yang terbagi dalam 
tiga dimensi utama: Konteks, Misi, serta Kejelasan dan 
Spesifisitas. Rincian indikator untuk prompt berkualitas 
tinggi (\textit{high-quality prompt}) disajikan dalam 
Tabel \ref{tab:prompt-manual}.

Penggunaan manual ini bertujuan untuk meminimalkan 
risiko halusinasi AI dan memastikan bahwa umpan balik 
yang dihasilkan oleh LLM memiliki struktur yang setara 
atau mendekati standar umpan balik ahli (\textit{expert feedback}).
...
BERTScore adalah metrik evaluasi otomatis untuk 
pembuatan teks yang menghitung skor kesamaan 
(\textit{similarity score}) untuk setiap token dalam kalimat 
kandidat dengan setiap token dalam kalimat referensi. 
Berbeda dengan metrik umum lainnya yang berbasis 
pencocokan n-gram (seperti BLEU) yang hanya melihat 
kecocokan bentuk kata secara eksak, BERTScore 
memanfaatkan penyematan kontekstual (\textit{contextual 
embeddings}) dari model BERT yang telah dilatih 
sebelumnya. Pendekatan ini dikembangkan untuk 
mengatasi kelemahan metrik n-gram yang sering kali 
gagal mencocokkan parafrase yang valid karena variasi 
kata, serta gagal menangkap dependensi jarak jauh dalam 
kalimat. Dengan menggunakan \textit{contextual embeddings}, 
BERTScore mampu mengukur kedekatan semantik antara 
kalimat yang dihasilkan model (kandidat) dan kalimat 
referensi, sehingga menghasilkan korelasi yang lebih 
kuat dengan penilaian manusia dibandingkan metrik 
tradisional \cite{zhang2020bertscore}. 

Berdasarkan representasi vektor kontekstual dari kalimat referensi 
$x = \langle x_1, \dots, x_k \rangle$ dan kalimat 
kandidat $\hat{x} = \langle \hat{x}_1, \dots, \hat{x}_l \rangle$, 
BERTScore menghitung Recall, Precision, dan skor F1 menggunakan 
\textit{cosine similarity}. Mengingat bahwa vektor-vektor tersebut telah 
dinormalisasi (sehingga \textit{cosine similarity} hanyalah hasil kali 
titik atau \textit{dot product} antara dua vektor), rumus perhitungannya 
adalah sebagai berikut:

\textbf{Recall ($R_{\text{BERT}}$):} 
Dihitung dengan mengambil nilai kemiripan maksimum (\textit{greedy matching}) 
dari setiap token di referensi ($x$) terhadap token-token 
di kandidat ($\hat{x}$), lalu dirata-rata.
$$R_{\text{BERT}} = \frac{1}{|x|} \sum_{x_i \in x} \max_{\hat{x}_j \in \hat{x}} \mathbf{x}_i^\top \hat{\mathbf{x}}_j$$

\textbf{Precision ($P_{\text{BERT}}$):} Dihitung dengan mengambil nilai kemiripan maksimum dari setiap token di kandidat 
($\hat{x}$) terhadap token-token di referensi ($x$), 
lalu dirata-rata.
$$P_{\text{BERT}} = \frac{1}{|\hat{x}|} \sum_{\hat{x}_j \in \hat{x}} \max_{x_i \in x} \mathbf{x}_i^\top \hat{\mathbf{x}}_j$$

\textbf{F1 Score ($F_{\text{BERT}}$):} Merupakan rata-rata 
harmonik dari Precision dan Recall.
$$F_{\text{BERT}} = 2 \frac{P_{\text{BERT}} \cdot R_{\text{BERT}}}{P_{\text{BERT}} + R_{\text{BERT}}}$$

\subsection{BARTScore}
BARTScore adalah metrik evaluasi untuk teks yang dihasilkan 
\textit{generated text} yang mengkonseptualisasikan evaluasi 
sebagai masalah pembuatan teks (\textit{text generation problem}). 
Berbeda dengan metrik berbasis pencocokan vektor seperti 
BERTScore yang membandingkan representasi semantik, BARTScore 
memanfaatkan probabilitas generasi dari model 
\textit{sequence-to-sequence} yang telah dilatih sebelumnya 
(seperti BART) untuk menilai kualitas teks. Ide dasarnya 
adalah model yang telah dilatih dengan baik seharusnya 
memberikan probabilitas yang tinggi pada teks yang berkualitas 
tinggi. Oleh karena itu, BARTScore mengukur seberapa besar 
kemungkinan model BART menghasilkan teks target (hipotesis) 
jika diberikan teks sumber (referensi atau input), atau 
sebaliknya. Pendekatan ini memungkinkan BARTScore untuk 
mengevaluasi berbagai aspek kualitas teks seperti kefasihan 
(\textit{fluency}), akurasi fakta (\textit{factuality}), dan kualitas 
parafrase tanpa memerlukan pelatihan ulang yang ekstensif \cite{yuan2021bartscore}.


BARTScore menghitung \textit{log-likelihood} tertimbang dari token-token dalam kalimat target ($y$) ketika 
dikondisikan pada kalimat sumber ($x$). Misalkan model memiliki 
parameter $\theta$, rumus dasarnya 
adalah:$$\text{BARTScore} = \sum_{t=1}^{m} w_t \log p(y_t | y_{<t}, x; \theta)$$
Di mana:
\begin{itemize}
    \item $y = \langle y_1, \dots, y_m \rangle$ adalah urutan \textit{token} dari teks yang sedang dinilai.
    \item $x$ adalah teks konteks atau referensi.
    \item $p(y_t \mid y_{<t}, x; \theta)$ adalah probabilitas bersyarat dari token $y_t$ yang diprediksi oleh model BART berdasarkan token sebelumnya ($y_{<t}$) dan konteks ($x$).
    \item $w_t$ adalah bobot untuk setiap token (biasanya $w_t = 1$ untuk rata-rata aritmatika standar).
\end{itemize}


\subsection{\textit{LLM-as-a-Judge}}
\textit{LLM-as-a-Judge} adalah paradigma evaluasi di mana 
LLM yang sangat kuat (seperti \textit{GPT-4}) digunakan 
sebagai evaluator otomatis untuk menilai kualitas output 
yang dihasilkan oleh model lain (biasanya model yang lebih 
kecil atau kurang berkinerja). Pendekatan ini muncul sebagai 
solusi atas kemacetan (bottleneck) dalam evaluasi manusia 
yang mahal dan lambat, serta keterbatasan metrik otomatis 
tradisional (seperti \textit{BLEU} atau \textit{ROUGE}) 
yang sering kali tidak berkorelasi baik dengan penilaian 
manusia pada tugas-tugas yang kompleks dan \textit{open-ended}.

Teori yang mendasarinya adalah bahwa LLM canggih telah 
mencapai tingkat kemampuan yang mendekati manusia dalam 
banyak tugas, sehingga mereka dapat memodelkan preferensi 
dan penilaian manusia dengan cukup akurat. Dalam konteks 
pendidikan (seperti dalam paper referensi), 
\textit{LLM-as-a-Judge} digunakan untuk menilai 
kualitas \textit{feedback} (pemrograman), seperti 
\textit{completeness}, \textit{perceptivity}, dan 
\textit{selectivity}. Studi menunjukkan bahwa LLM seperti 
GPT-4 memiliki kesepakatan (\textit{agreement}) 
yang moderat hingga tinggi dengan penilai ahli manusia, 
menjadikannya proksi yang layak untuk evaluasi skala 
besar. \cite{koutcheme2024openfeedback}

Tidak seperti metrik berbasis \textit{n-gram} atau 
\textit{embedding} yang memiliki rumus matematika 
tertutup yang statis, \textit{LLM-as-a-Judge} 
diformulasikan sebagai fungsi pemetaan probabilistik 
yang dikondisikan pada sebuah \textit{prompt}. Jika 
kita mendefinisikan fungsi evaluasi $f_{\theta}$ yang 
diparameterisasi oleh LLM (misalnya GPT-4), maka skor 
atau label evaluasi $E$ diperoleh melalui:
$$E = f_{\theta}(\mathcal{I}, \mathcal{C}, \mathcal{R}, \mathcal{S})$$
Di mana:
\begin{itemize}
    \item $\mathcal{I}$: Instruksi evaluasi atau \textit{rubrik} (misalnya, ``Nilai apakah umpan balik ini membantu'').
    \item $\mathcal{C}$: Konteks masalah (misalnya, deskripsi soal, kode siswa yang salah, solusi model).
    \item $\mathcal{R}$: Respons atau output yang dievaluasi (misalnya, umpan balik yang dihasilkan oleh model target).
    \item $\mathcal{S}$: Kriteria penilaian (misalnya, skala 1--5 atau label biner Ya/Tidak (1/0)).
\end{itemize}

Proses ini biasanya menggunakan \textit{greedy decoding} 
(memilih token dengan probabilitas tertinggi) untuk 
memastikan determinisme hasil evaluasi:
$$E = \operatorname*{argmax}_{y \in \mathcal{Y}} P_{\theta}(y \mid \text{Prompt}(\mathcal{I}, \mathcal{C}, \mathcal{R}))$$

Contoh \textit{prompt} untuk menilai kelengkapan umpan 
balik yang berasal dari
literatur referensi adalah sebagai berikut 
\cite{koutcheme2024openfeedback}:

\begin{lstlisting}
1. You are a computer science professor teaching introductory programming using Dart.

2. Below is a problem description, a model solution, and an incorrect program written by a student. You are also provided with the feedback generated by GPT-3.5. Your task is to evaluate the quality of the feedback (by saying yes or no) to ensure it adheres to the multiple criteria outlined below. For each criterion, provide your answer in a separate line with the format '(CRITERIA_NUMBER): Yes/No'. Do not provide comments, but be attentive to the problem description requirements.
...
(3) Does not identify non-existent issues
\end{lstlisting}
