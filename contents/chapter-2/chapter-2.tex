...
Para penulis yaitu, Juliette Woodrow, Sanmi Koyejo, dan Chris Piech (Stanford), mengajukan makalah 
...
Penulis menyebutnya “\textit{Feedback Alignment Challenge}” yaitu tantangan menyesuaikan keluaran 
...
Solusi yang ditawarkan adalah Direct Preference Optimization (DPO) dengan guru/TA di 
dalam loop. Inti dari DPO adalah selama proses penilaian, 
...
probabilitas keluaran yang disukai dibanding yang tidak disukai. Penulis merancang 
pipa tiga tahap—koleksi preferensi saat grading, pelatihan DPO antar tugas, dan 
inferensi untuk tugas berikutnya—mewujudkan sistem self-improving yang semakin selaras 
...
menggunakan Hugging Face DPOTrainer. Proses pelatihan berlangsung sekitar tujuh jam 
...
Penulis menguji pendekatan ini pada dua skenario. Pertama, studi terkontrol 
...
Penulis juga mencatat potensi \textit{confounding variable} dalam deployment 
...
Untuk evaluasi, penulis memadukan penilaian manusia secara buta dan penilaian otomatis 
...
Di luar itu, penulis memperkenalkan \textit{critic model} baru dengan tiga dimensi yaitu 
...
\subsection{Peningkatan \textit{Feedback} LLM dengan \textit{Prompt Engineering} Berbasis \textit{Learning Analytics}}
...
Sebagai respons, penulis mengembangkan sebuah manual prompt berbasis teori. 
...
(rendah–menengah–tinggi), penulis menghasilkan 20 umpan balik per prompt 
...
pendekatan ini, penulis tidak hanya menilai “siapa yang lebih baik”, tetapi 
...
Penulis menekankan bahwa kualitas prompt harus diajarkan sebagai 
keterampilan bagi pendidik; manual yang mereka kembangkan dapat dijadikan 
panduan praktis. Namun, mereka juga mengingatkan keterbatasan—misalnya, 
...
sekadar memperbesar parameter. Penulis tetap mencatat keterbatasan model yang 
...
Untuk menjawab celah tersebut, penulis merancang CodeRunner Agent, sebuah alat 
...
paper rancangan; penulis merencanakan \textit{pilot} jangka pendek dan 
...
diaktifkan. Di luar metrik kuantitatif, penulis juga merencanakan studi kelas 
...
Penulis melakukan dua studi evaluasi. Pertama, studi lapangan di kelas TIK tingkat 
...
target. Penulis menekankan pentingnya menjaga keseimbangan antara informasi 
...
melalui Groq API tanpa pelatihan ulang. Pendekatan \textit{prompt}/\textit{context engineering} 
...
pengambilan tindakan cepat oleh pengajar. Komponen ini belum tercakup 
pada studi yang berfokus pada \textit{prompt} semata. Beberapa hal yang ada di 
...
Kontrol pedagogis tetap dipertahankan melalui penilaian kualitatif yang akan
...
antara teks yang dihasilkan dan teks referensi (\textit{ground truth})[1]. Namun, 
...
tetapi menggunakan kosakata yang berbeda[2]. Berbagai studi telah menunjukkan bahwa 
korelasi metrik ini dengan penilaian manusia sering kali rendah, terutama dalam 
tugas-tugas yang menuntut keragaman ekspresi seperti simplifikasi teks atau dialog[3].
...
terhadap konteks. Untuk pendekatan kesamaan semantik, metrik berbasis 
\textit{embedding} kontekstual 
seperti BERTScore dan BARTScore menjadi pilihan utama [1][2]. Lalu,
...
dengan penilaian kualitas oleh manusia[1]. Di sisi lain, pendekatan 
...
meniru proses evaluasi oleh manusia pada skala yang lebih besar[4].
...
Dalam paper ini [1] Metrik ini bekerja dengan mengubah setiap token dalam kalimat 
...
Sementara itu, BARTScore [1] menawarkan filosofi yang berbeda secara fundamental 
...
kerangka kerja terstruktur seperti G-Eval telah dikembangkan [1]. 
G-Eval menggunakan 
...
G-Eval terlebih dahulu menginstruksikan LLM untuk menghasilkan l
angkah-langkah evaluasi berdasarkan kriteria yang diberikan pengguna, 
...
dibandingkan metode LLM-penilai yang tidak terstruktur. Pada penelitian [2],
...
prinsip-prinsip teoretis yang telah mapan. Mengacu pada Shute (2008), 
...
Selanjutnya, dengan mengadopsi model dari Hattie dan Timperley (2007), 
...
Self-Determination Theory (Deci \& Ryan, 1985) dan prinsip penguatan 
positif (Fredrickson, 2001). Penilai manusia akan mengevaluasi apakah 
